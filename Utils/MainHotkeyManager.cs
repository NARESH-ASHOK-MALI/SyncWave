...
namespace SyncWave.Utils
{
    ...
    /// This is a separate instance for the main application hotkey (show/hide main window).
    /// </summary>
    public class MainHotkeyManager : IDisposable
    {
        private const int HOTKEY_ID = 9002; // Different ID to avoid conflict with Flyout hotkey
        ...
        public string CurrentHotkeyString { get; private set; } = "Ctrl+Shift+Alt+Space";
        ...
        public static string LoadHotkeyConfig()
        {
            ...
            {
                Logger.Warn($"Failed to load main hotkey config, using default: {ex.Message}");
            }
        ...
        public static bool SaveHotkeyConfig(string hotkeyString)
        {
            ...
            {
                Logger.Error($"Failed to save main hotkey config '{hotkeyString}'", ex);
                return false;
            }
        }
        ...
        private bool RegisterCurrentHotkey()
        {
            ...
            {
                Logger.Warn($"Could not parse main hotkey string '{CurrentHotkeyString}'. Falling back to Ctrl+Shift+Alt+Space.");
                CurrentHotkeyString = "Ctrl+Shift+Alt+Space";
            ...
            {
                _isRegistered = true;
                Logger.Info($"Successfully registered global main hotkey: {CurrentHotkeyString}");
            }
            else
            {
                int error = Marshal.GetLastWin32Error();
                Logger.Warn($"Failed to register global main hotkey '{CurrentHotkeyString}' (Win32 Error: {error}). The hotkey combination may be claimed by another application.");
            }
        ...
        private IntPtr WndProc(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
        {
            ...
            {
                Logger.Info($"Global main hotkey '{CurrentHotkeyString}' triggered.");
